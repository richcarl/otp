...
-spec erase(Key :: term(), Tab :: tab_name()) -> Tab :: tab_name().
...
%% function returns the given `Tab' on success and throws an exception on
%% error.
%% @see delete/2

erase(Key, Tab) ->
    case delete(Tab, Key) of
        ok -> Tab;
        {error, Reason} -> erlang:error({delete, Reason})
    end.
...
-spec last(Name) -> Key | '$end_of_table' when
      Name :: tab_name(),
      Key :: term().
...
-spec is_empty(Tab :: tab_name()) -> boolean().
...
-spec is_key(Key :: term(), tab_name()) -> boolean().
...
-spec prev(Name, Key1) -> Key2 | '$end_of_table' when
      Name :: tab_name(),
      Key1 :: term(),
      Key2 :: term().
...
%% always fails because option {name, Name} is expected
new() ->
    new([]).

-spec new(Opts :: [Option :: term()]) -> tab_name().
...
new(Options) ->
    case lists:keyfind(name, 1, Options) of
        {name, Name} ->
            case open_file(Name, proplists:delete(name, Options)) of
                {ok, Name} -> Name;
                {error, R} -> erlang:error({open_file,R})
            end;
        _ ->
            erlang:error(badarg)
    end.
...
open_file(File) ->
    case dets_server:open_file(ensure_list(File)) of
        badarg -> % Should not happen.
...
defaults(Tab, Args) ->
    Defaults0 = #open_args{file = ensure_list(Tab),
                           type = set,
                           keypos = 1,
                           repair = true, 
...
ensure_list(T) when is_atom(T) -> atom_to_list(T);
ensure_list(T) -> T.
...
repl({file, File}, Defs) ->
    Defs#open_args{file = ensure_list(File)};
...
-spec to_list(Tab :: tab_name()) -> List when
      List :: [{Key :: term(), Value :: term()}].

%% @doc Convert a table to a list of pairs. This function converts the table
%% to a list representation. The result is in the order defined by {@link
%% foldl/3}.
...
-spec from_list(List, Opts :: [Option :: term()]) -> Tab :: tab_name() when
      List :: [{Key :: term(), Value :: term()}].
...
-spec from_orddict(List) -> Tab :: tab_name() when
      List :: [{Key :: term(), Value :: term()}].
...
-spec from_orddict(List, Opts :: [Option :: term()]) -> Tab :: tab_name() when
      List :: [{Key :: term(), Value :: term()}].
...
-spec keys(Tab :: tab_name()) -> Keys :: [term()].

%% @doc Return all keys in a table. This function returns a list of all
%% keys in `Tab'. The resulting list is unordered.

keys(Tab) ->
    foldr(fun (Key, _Val, Keys) -> [Key|Keys] end, [], Tab).

-spec values(Tab :: tab_name()) -> [Val :: term()].
...
-spec values(Key :: term(), Tab :: tab_name()) -> [Value :: term()] | [].
...
values(Key, Tab) -> lookup(Tab, Key).

-spec get(Key, Tab :: tab_name()) -> Value when
      Key :: term(),
      Value :: term().
...
-spec get(Key, Default, Tab :: tab_name()) -> Value when
      Key :: term(),
      Default :: term(),
      Value :: term().
...
%% It sucks that dets:{first,last}/1 and dets:{next,prev}/2 on an unordered
%% table traverse the table in the same order (and so does dets:foldr/3),
%% but maybe that can be changed in the future. Hence, we don't try to do
%% anything fancy with last_key/next_key here.
last_key(Tab) ->
    case last(Tab) of
        '$end_of_table' -> error;
        Key -> {ok, Key}
