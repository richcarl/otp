...
%% Usage: mnesia:create_table(Tab, [{disc_only_copies, Nodes}, ...]).

-module(mnesia_disc_only_copies).
...
-define(ALIAS, disc_only_copies).

%% enable debugging messages through mnesia:set_debug_level(debug)
-ifndef(MNESIA_DISC_ONLY_COPIES_NO_DBG).
-define(dbg(Fmt, Args),
...
check_definition(Alias, Tab, Nodes, Props) ->
    %% FIXME: rewrite for disc_only_copies
    Id = {Alias, Nodes},
    Props1 = lists:map(
               fun({type, T} = P) ->
